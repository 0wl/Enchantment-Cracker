...
- [x] Easter egg: make the needle of the compass icon on the Plan tab point towards the mouse cursor.
  - Done a safe way: the Plan tab's compass icon has a bright needle drawn over it that points at
    the cursor (`Widgets.TabButton.drawNeedle`, flagged for the Plan tab in `CrackerScreen`). This
    avoids replacing vanilla's global compass model property, which would risk real compasses.
...
- [x] Make the text easier to read: dark text with a dark drop shadow is hard to read.
  - `Mc.label`/`Mc.centeredLabel` drop the shadow for dark colours; button labels use them. Tab
    labels use a near-black colour on the greyer unselected tabs, and the table-overlay greys were
    lightened.
...
- [x] Auto drop: items should land in front of the player, not at their feet.
  - `AutoDropper` levels the view (pitch 0) while dropping and restores it afterwards, so items are
    thrown forward instead of underfoot.
